...
@MainActor func runServiceTests() async throws {
    ...
    card.showWindow(nil)
    precondition(card.window!.isVisible)
    card.windowDidResignKey(Notification(name:NSWindow.didResignKeyNotification,object:card.window))
    precondition(!card.window!.isVisible)
    card.showWindow(nil)
    precondition(card.window!.isVisible, "Card can reopen after focus dismissal")
