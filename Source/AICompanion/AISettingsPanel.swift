...
@MainActor
final class AISettingsPanel: NSWindowController {
    ...
    private let resetFormat = NSPopUpButton()
    ...
    init(service:AICompanionService) {
        ...
        window.title = service.demo ? "奶灰 · AI 提醒设置（演示）" : "奶灰 · AI 提醒设置"
        ...
        let title = Self.label("提醒设置",size:23,weight:.semibold)
        ...
        root.addArrangedSubview(Self.label("Codenotch 规则 · 主额度已用 80% / 100% 提醒，周额度耗尽提醒",size:12))
        let formatRow = NSStackView(); formatRow.spacing = 8
        formatRow.addArrangedSubview(Self.label("重置时间",size:12))
        resetFormat.addItems(withTitles:["自动日期（Codenotch 默认）","剩余倒计时"])
        resetFormat.selectItem(at:(service.settings.resetTimeFormat ?? .automatic) == .automatic ? 0 : 1)
        resetFormat.target = self; resetFormat.action = #selector(changeResetFormat)
        formatRow.addArrangedSubview(resetFormat); root.addArrangedSubview(formatRow)
    ...
    func refresh() {
        ...
        status.stringValue = service.demo ? "演示数据，不会读取账号、发送通知或保存设置。" : service.isMuted ? "已暂停主动提醒；额度和任务继续更新。" : "点击连接后开始读取，额度和提醒规则与 Codenotch v1.14.0 对齐。"
        ...
        for (index,p) in AIProvider.allCases.enumerated() {
            ...
            if p == .claude, service.settings.enabled.contains(p), [.needsAuth,.accessDenied].contains(service.readings[p]?.status ?? .ok) {
                header.addArrangedSubview(button("允许钥匙串访问",#selector(allowClaude)))
            }
    ...
    @objc private func changeResetFormat() {
        service.updateSettings { $0.resetTimeFormat = resetFormat.indexOfSelectedItem == 0 ? .automatic : .remaining }
    }
    @objc private func allowClaude() { service.authorizeClaude() }
}
